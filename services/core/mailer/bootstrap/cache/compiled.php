...
class Validator implements ValidatorContract
{
    ...
    protected $dependentRules = ['After', 'AfterOrEqual', 'Before', 'BeforeOrEqual', 'Confirmed', 'Different', 'ExcludeIf', 'ExcludeUnless', 'ExcludeWith', 'ExcludeWithout', 'Gt', 'Gte', 'Lt', 'Lte', 'AcceptedIf', 'DeclinedIf', 'RequiredIf', 'RequiredUnless', 'RequiredWith', 'RequiredWithAll', 'RequiredWithout', 'RequiredWithoutAll', 'Prohibited', 'ProhibitedIf', 'ProhibitedUnless', 'Prohibits', 'Same', 'Unique'];
    ...
    protected function validateUsingCustomRule($attribute, $value, $rule)
    {
        ...
        if (!$rule->passes($attribute, $value)) {
            ...
            foreach ($messages as $key => $message) {
                ...
                $this->messages->add($key, $this->makeReplacements($message, $key, get_class($rule), []));
            }
        }
    }
...
class PostgresBuilder extends Builder
{
    ...
    public function hasTable($table)
    {
        ...
        $table = $this->connection->getTablePrefix() . $table;
        return count($this->connection->select($this->grammar->compileTableExists(), [$database, $schema, $table])) > 0;
    }
    public function dropAllTables()
    {
        $tables = [];
    ...
    public function getColumnListing($table)
    {
        [$database, $schema, $table] = $this->parseSchemaAndTable($table);
        $table = $this->connection->getTablePrefix() . $table;
        $results = $this->connection->select($this->grammar->compileColumnListing(), [$database, $schema, $table]);
        return $this->connection->getPostProcessor()->processColumnListing($results);
    }
